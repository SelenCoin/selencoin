...
unsigned int nStakeMaxAge = 60 * 60 * 24; // 24 hours
...
const string strMessageMagic = "Selen signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 100)
        nSubsidy = 20000 * COIN;
    else if(pindexBest->nHeight <= 500) // Make sure our chain is moving.
        nSubsidy = 1 * COIN;
...
const int YEARLY_BLOCKCOUNT = 1576800;
// miner's coin stake reward based on coin age spent (coin-days)
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    if (pindexBest->nHeight > (YEARLY_BLOCKCOUNT*100)) // 100 years.
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "selen", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Selen coin started at 06.11.16";
        CTransaction txNew;
        txNew.nTime = 1478381592;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
